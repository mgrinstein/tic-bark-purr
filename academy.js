...
const winningArray = [
    [
        ...
        [0, 1],
        [0, 2],
    ], // 1st vertical streak (row 0)
    [
        [1, 0],
        [1, 1],
        [1, 2],
    ], // 2nd vertical streak (row 1)
    [
        [2, 0],
        [2, 1],
        [2, 2],
    ], // 3rd vertical streak (row 2)
    [
        [0, 0],
        [1, 0],
        [2, 0],
    ], // 1st horizontal streak (column 0)
    [
        [0, 1],
        [1, 1],
        [2, 1],
    ], // 2nd horizontal streak (column 1)
    [
        [0, 2],
        [1, 2],
        [2, 2],
    ], // 3rd horizontal streak (column 2)
    [
        [0, 0],
        [1, 1],
        [2, 2],
    ], // 1st diagonal streak (top left to bottom right)
    [
        [0, 2],
        [1, 1],
        [2, 0],
    ], //  2nd diagonal streak (top right to bottom left)
];
...
const winnerDictionary = {
    crosses: "The winner is 🐶",
    noughts: "The winner is 🐱",
    nobody: "Draw!",
};
// display next player
function displayNextPlayer(isGameOver) {
    ...
    if (!isGameOver) {
        currentPlayerName.innerText = currentPlayer
            ? nextPlayerDictionary[currentPlayer]
            : defaultPlayer;
    } else {
...
function takeTurn(row, column) {
    ...
    if (isGameOver === false && myTTTGrid[row][column] === null) {
        if (currentPlayer === "crosses") {
            // Update board with new move & whose turn it is now
...
function movesLeft(grid) {
    // check if there are moves left on grid
    for (row in grid) {
...
function checkWinner() {
    ...
    for (let streak = 0; streak < winningArray.length; streak++) {
        streakCell_1 =
            myTTTGrid[winningArray[streak][0][0]][winningArray[streak][0][1]];
        streakCell_2 =
            myTTTGrid[winningArray[streak][1][0]][winningArray[streak][1][1]];
        streakCell_3 =
            myTTTGrid[winningArray[streak][2][0]][winningArray[streak][2][1]];
        if (
            streakCell_1 === "cross" &&
            streakCell_2 === "cross" &&
            streakCell_3 === "cross"
